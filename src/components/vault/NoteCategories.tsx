    ...
    <div className="min-h-screen bg-gradient-to-b from-slate-50 via-white to-indigo-50 px-4 py-8">
      <div className="max-w-6xl mx-auto">
        ...
        <div className="flex items-center justify-between mb-8">
          ...
          <button
            onClick={() => navigate("/vault/notes/add-category")}
        ...
        {isLoading ? (
          ...
          <div className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-4 md:gap-6">
            {categories.map((category) => (
              <CategoryCard
                ...
                onClick={() => navigate(`/vault/notes/${category._id}`)}
              />
            ))}
          </div>
        ) : (
          <div className="flex flex-col items-center justify-center py-20 text-center">
            ...
            <p className="text-gray-500 mb-6 max-w-sm">
              Create categories like "Work Notes", "Personal Ideas" to organize
              your thoughts
            </p>
            <button
              onClick={() => navigate("/vault/notes/add-category")}
              className="flex items-center gap-2 px-6 py-3 bg-gradient-to-r from-indigo-500 to-indigo-600 text-white font-medium rounded-xl hover:from-indigo-600 hover:to-indigo-700 transition-all shadow-md"
            >
              <svg
                className="w-5 h-5"
                fill="none"

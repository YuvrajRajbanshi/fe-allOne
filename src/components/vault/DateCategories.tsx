...
const DateCategories: React.FC = () => {
  const navigate = useNavigate();
  const userId = useSelector((state: RootState) => state.user.userId);
  const [categories, setCategories] = useState<DateCategoryType[]>([]);
  ...
  const fetchCategories = async () => {
    try {
      ...
      console.error("Failed to fetch date categories:", error);
    } finally {
      setIsLoading(false);
    }
  };
          ...
          <div className="flex items-center gap-4">
            ...
            <div>
              ...
              <p className="text-gray-500 text-sm">
                Organize your special moments
              </p>
            </div>
          </div>
          <button
            onClick={() => navigate("/vault/dates/add-category")}
        ...
        {isLoading ? (
          ...
          <div className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-4 md:gap-6">
            {categories.map((category) => (
              <CategoryCard
                ...
                type={category.type}
                onClick={() => navigate(`/vault/dates/${category._id}`)}
              />
            ))}
          </div>
        ) : (
          <div className="flex flex-col items-center justify-center py-20 text-center">
            ...
            <p className="text-gray-500 mb-6 max-w-sm">
              Create categories like "Birthdays", "Anniversaries" to organize
              your important dates
            </p>
            <button
              onClick={() => navigate("/vault/dates/add-category")}
              className="flex items-center gap-2 px-6 py-3 bg-gradient-to-r from-orange-500 to-orange-600 text-white font-medium rounded-xl hover:from-orange-600 hover:to-orange-700 transition-all shadow-md"
            >
              <svg
                className="w-5 h-5"
                fill="none"
